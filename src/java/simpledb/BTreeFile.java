...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		if (rootid.pgcateg()==BTreePageId.LEAF) return (BTreeLeafPage) getPage(tid,dirtypages, rootid, perm);
		BTreePage now=(BTreeInternalPage) getPage(tid,dirtypages, rootid, Permissions.READ_ONLY);
		while (now.pid.pgcateg()==BTreePageId.INTERNAL) {
			BTreeInternalPage temp=(BTreeInternalPage) now;
			Iterator<BTreeEntry> it = temp.iterator();
			BTreeEntry entry=null;
			if (f==null) entry=it.next();
			else 
			while (it.hasNext()) {
				entry=it.next();
				if (entry.getKey().compare(Predicate.Op.GREATER_THAN_OR_EQ, f)) break;
			}
			if (f==null || entry.getKey().compare(Predicate.Op.GREATER_THAN_OR_EQ, f)) {
				if (entry.getLeftChild().pgcateg()==BTreePageId.INTERNAL)
					now=(BTreePage) getPage(tid,dirtypages, entry.getLeftChild(),Permissions.READ_ONLY);
				else if (entry.getLeftChild().pgcateg()==BTreePageId.LEAF)
					now=(BTreePage) getPage(tid,dirtypages, entry.getLeftChild(),perm);
			}
			else {
				if (entry.getLeftChild().pgcateg()==BTreePageId.INTERNAL)
					now=(BTreePage) getPage(tid,dirtypages, entry.getRightChild(),Permissions.READ_ONLY);
				else if (entry.getLeftChild().pgcateg()==BTreePageId.LEAF)
					now=(BTreePage) getPage(tid,dirtypages, entry.getRightChild(),perm);
			}
		}
        return (BTreeLeafPage) now;
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		if (x.getField(keyField).compare(Predicate.Op.LESS_THAN_OR_EQ, field)) {//x is the tuple
			ret=newpage;
			newentry=new BTreeEntry(x.getField(keyField),page.pid,newpage.pid);
			parent=getParentWithEmptySlots(tid,dirtypages,page.getParentId(),x.getField(keyField));
			parent.insertEntry(newentry);
		}
		else {
			Tuple y=it.next();
			if (y.getField(keyField).compare(Predicate.Op.LESS_THAN_OR_EQ, field)) {//field is the tuple
				ret=newpage;
				newentry=new BTreeEntry(field,page.pid,newpage.pid);
				...
				ret=page;
				newentry=new BTreeEntry(x.getField(keyField),page.pid,newpage.pid);
				parent=getParentWithEmptySlots(tid,dirtypages,page.getParentId(),y.getField(keyField));
				parent.insertEntry(newentry);
			}
				
		}
		page.setParentId(parent.pid);
		newpage.setParentId(parent.pid);
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numslots=page.getMaxEntries();
		BTreeEntry x=null;
		for (int i=0;i<(numslots+1)/2;++i) {
			x=it.next();
			page.deleteKeyAndRightChild(x);
			BTreePage pg=(BTreePage) getPage(tid,dirtypages,x.getRightChild(),Permissions.READ_WRITE);
			pg.setParentId(newpage.pid);		
			dirtypages.put(pg.pid, pg);
			newpage.insertEntry(x);
		}
		BTreeInternalPage parent=null;
		BTreeEntry newentry=null;
		if (x.getKey().compare(Predicate.Op.LESS_THAN_OR_EQ, field)) {//x is the Entry
			ret=newpage;
			newentry=new BTreeEntry(x.getKey(),page.pid,newpage.pid);
			parent=getParentWithEmptySlots(tid,dirtypages,page.getParentId(),x.getKey());
			newpage.deleteKeyAndLeftChild(x);
			parent.insertEntry(newentry);
		}else {//y is the entry
/*				BTreeEntry y=it.next();
				ret=newpage;
				newentry=new BTreeEntry(y.getKey(),page.pid,newpage.pid);
				parent=getParentWithEmptySlots(tid,dirtypages,page.getParentId(),y.getKey());
				page.deleteKeyAndRightChild(y);
				BTreePage pg=(BTreePage) getPage(tid,dirtypages,y.getRightChild(),Permissions.READ_WRITE);
				pg.setParentId(newpage.pid);	
				dirtypages.put(pg.pid, pg);
				parent.insertEntry(newentry);*/
			ret=page;
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		if (!isRightSibling) {
			...
			while (sibling.getNumTuples()>page.getNumTuples()) {
				x=it.next();
				sibling.deleteTuple(x);
				page.insertTuple(x);
			}
			entry.setKey(x.getField(keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry rm=it.next();
		BTreeEntry lm=it2.next();
		BTreeEntry pull=new BTreeEntry(parentEntry.getKey(),rm.getRightChild(),lm.getLeftChild());
		page.insertEntry(pull);
		while (leftSibling.getNumEntries()-1>page.getNumEntries()) {
			BTreePage child=(BTreePage) getPage(tid, dirtypages, rm.getRightChild(), Permissions.READ_WRITE);
			dirtypages.put(child.getId(), child);
			child.setParentId(page.pid);
			leftSibling.deleteKeyAndRightChild(rm);
			page.insertEntry(rm);
			rm=it.next();
		}
		BTreePage child=(BTreePage) getPage(tid, dirtypages, rm.getRightChild(), Permissions.READ_WRITE);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry lm=it.next();
		BTreeEntry rm=it2.next();
		BTreeEntry pull=new BTreeEntry(parentEntry.getKey(),rm.getRightChild(),lm.getLeftChild());
		page.insertEntry(pull);
		while (rightSibling.getNumEntries()-1>page.getNumEntries()) {
			BTreePage child=(BTreePage) getPage(tid, dirtypages, lm.getLeftChild(), Permissions.READ_WRITE);
			dirtypages.put(child.getId(), child);
			child.setParentId(page.pid);
			rightSibling.deleteKeyAndLeftChild(lm);
			page.insertEntry(lm);
			lm=it.next();
		}
		BTreePage child=(BTreePage) getPage(tid, dirtypages, lm.getLeftChild(), Permissions.READ_WRITE);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		if (rightPage.getRightSiblingId()!=null) {
			BTreeLeafPage right=(BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
			dirtypages.put(right.pid, right);
			right.setLeftSiblingId(leftPage.pid);
			leftPage.setRightSiblingId(right.pid);
		}
		else {
		...
		while (it.hasNext()) {
			Tuple x=it.next();
			rightPage.deleteTuple(x);
			leftPage.insertTuple(x);
		}
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		dirtypages.put(rightPage.pid, rightPage);
		dirtypages.put(parent.pid, parent);
		Iterator<BTreeEntry> itl=leftPage.reverseIterator();
		Iterator<BTreeEntry> itr=rightPage.iterator();
		BTreeEntry lm=itl.next();
		BTreeEntry rm=itr.next();
		BTreeEntry pull=new BTreeEntry(parentEntry.getKey(),lm.getRightChild(),rm.getLeftChild());
		leftPage.insertEntry(pull);
		while (true) {
			BTreePage child=(BTreePage) getPage(tid,dirtypages,rm.getLeftChild(),Permissions.READ_WRITE);
			dirtypages.put(child.pid, child);
			child.setParentId(leftPage.pid);
			rightPage.deleteKeyAndLeftChild(rm);
			leftPage.insertEntry(rm);
